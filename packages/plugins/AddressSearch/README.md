...
| likeFilterAttributes | Record<string, string>? | Key/value pairs that are mapped to custom attributes of the isLike operators used in filters. The default is {wildCard: "\*",singleChar: ".",escapeChar: "!"} which results in <ogc:PropertyIsLike wildCard="*" singleChar="." escapeChar="!"> |

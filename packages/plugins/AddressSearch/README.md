...
| likeFilterAttributes | Record<string, string>? | As specified by the [OGC-Standards for filters](https://schemas.opengis.net/filter/) the PropertyIsLike operator requires three attributes (e.g. in WFS 2.0.0: wildCard, singleChar and escapeChar). But these may vary in value and (with other WFS verions) also in property definition. Therefore it is possible to configure here the values of the attributes needed for WFS 2.0.0 and also to add custom attributes needed for other versions. The default is `{wildCard: "\*",singleChar: ".",escapeChar: "!"}`. Configuration examples: WFS 2.0.0 `{wildCard: "%",singleChar: "\*",escapeChar: "\"}`; WFS 1.0.0 `{wildCard: "\%",singleChar: "*",escape: "\"}` |
...
| useRightHandWildcard? | boolean? | By default, if searching for an "input", it is sent as "input{wildCard}". Where {wildcard} stands for the configured wildcard charakter (see likeFilterAttributes). Only the value in the key `wildcard` will be used here and the default is *. This behaviour can be deactivated by setting this parameter to `false`.                                                                                                                                                                                                                                                                                                                                               |

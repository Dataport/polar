...
import actions, { drawSource } from './actions'
...
const storeModule: PolarModule<DrawState, DrawGetters> = {
  ...
  getters: {
    ...
    selectableModes(_, { configuration }) {
      const includesWrite = configuration?.selectableDrawModes?.includes('Text')
    ...
    updateFeatures(state) {
      const features = drawSource.getFeatures().map((feature) => {
        ...
        const jsonFeature: Feature = {
          type: 'Feature',
          properties: feature.get('text') ? { text: feature.get('text') } : {},
          geometry: {

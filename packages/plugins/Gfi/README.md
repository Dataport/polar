...
| fieldName | type | description |
| - | - | - |
| mode | 'visible' \| 'known' | Whether to show only features currently visible in the map view's bounding box or to display all known (i.e. loaded) features. In the latter case, if you desire to display all features of a layer (seen or not), set its loading strategy to `'all'`. |

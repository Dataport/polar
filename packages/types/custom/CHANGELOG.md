...
- Feature: Add new exported types `RequestGfiMode` and `GfiFeatureInformation`.
- Fix: Add missing parameter `maxFeatures` to `GfiLayerConfiguration`.
- Fix: Add missing parameter `mode` to `GfiConfiguration`.
- Fix: Add missing parameter `gfiMode` to `LayerConfiguration`.

...
import { knownHatches, getPolygonFillHatch } from './hatches'
...
const maybeHatch = (fill: string) =>
  knownHatches.includes(fill)
    ? console.error(getPolygonFillHatch({ pattern: fill }).toDataURL()) ||
      `url('${getPolygonFillHatch({ pattern: fill }).toDataURL()}')`
    : fill
...
<svg width="${imgSize[0]}" height="${
  ...
  <defs>
    <pattern id="img" patternUnits="userSpaceOnUse" width="50" height="50">
      <image href="${maybeHatch(fill)}" width="50" height="50"/>
    </pattern>
  </defs>
  <path

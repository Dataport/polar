...
| styleId | string? | Id of the used style. May lead to unexpected results if the layer is also configured to be used with `mapConfiguration.extendedMasterportalapiMarkers`. For more information and and example see `mapConfiguration.featureStyles`. Defaults and fallbacks to OpenLayers default styling. |

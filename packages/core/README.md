...
| stylePath | string? | This path will be used to create the link node in the client itself. It defaults to `'./style.css'`. In most client configurations, this parameter will have to updated with a different path as it is a relative path based on the directory where `createMap` is called. |

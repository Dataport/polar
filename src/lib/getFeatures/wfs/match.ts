...
import type { KeyValueSetArray } from '../types'

type Separator = string
type Slot = RegExpMatchArray
 ...
 */
export function getBlocks(pattern: string): Block[] {
...
const sortComparableMatches = (comparableA, comparableB) => {
	...
	if (comparableA.uninterpreted > comparableB.uninterpreted) {
		return 1
	}
	if (comparableA.uninterpreted < comparableB.uninterpreted) {
		return -1
	...
	if (comparableA.index > comparableB.index) {
		return 1
	}
	if (comparableA.index < comparableB.index) {
		return -1
...
const sortMatches = (
	...
	const comparableMatches = matches.map((match, index) => ({
		...
		patternLengthDiff:
			((patterns[index] as string).match(/{{/g) || []).length - match.length,
		index,
	}))
...
export const match = (
	...
	if (!patterns) {
		throw new Error(
			'Parameter "patterns" is missing on wfs configuration for pattern-based search.'
		)
	}
	if (!patternKeys) {
		throw new Error(
			'Parameter "patternKeys" is missing on wfs configuration for pattern-based search.'
		)
	}
	...
	patterns.forEach((pattern) => {
		...
		patternBlocks.forEach((block) => {
			...
			if (Array.isArray(block)) {
				// Slot case
				const patternName = block[1] as string
				const patternRegExp = patternKeys[patternName] as string
				const value = traverseInput.match(new RegExp('^' + patternRegExp))
				if (value !== null) {
					const capturingGroupContent = value[1] as string

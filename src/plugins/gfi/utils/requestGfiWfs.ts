...
>): Promise<GeoJsonFeature[]> => {
	...
	featureUrl.searchParams.set('version', version as string)
	featureUrl.searchParams.set('request', 'GetFeature')
	featureUrl.searchParams.set(typeName, featureType as string)
	featureUrl.searchParams.set('srsName', code)
	...
	return (
		fetch(featureUrl, { method: 'GET' })
			...
			//  and get rid of this @ts-expect-error
			// @ts-expect-error | Better typing is needed here

import{i as V,c as h,a as i,e as P,b as p,d as Z,f as A,g as H,h as W,j as R,k as a,l as u,m as X,n as G,o as e,p as F}from"./runtime-core.esm-bundler-BwRGlRWM.js";let o;const S=typeof window<"u"&&window.trustedTypes;if(S)try{o=S.createPolicy("vue",{createHTML:M=>M})}catch{}const b=o?M=>o.createHTML(M):M=>M,J="http://www.w3.org/2000/svg",_="http://www.w3.org/1998/Math/MathML",j=typeof document<"u"?document:null,O=j&&j.createElement("template"),K={insert:(M,t,T)=>{t.insertBefore(M,T||null)},remove:M=>{const t=M.parentNode;t&&t.removeChild(M)},createElement:(M,t,T,N)=>{const I=t==="svg"?j.createElementNS(J,M):t==="mathml"?j.createElementNS(_,M):T?j.createElement(M,{is:T}):j.createElement(M);return M==="select"&&N&&N.multiple!=null&&I.setAttribute("multiple",N.multiple),I},createText:M=>j.createTextNode(M),createComment:M=>j.createComment(M),setText:(M,t)=>{M.nodeValue=t},setElementText:(M,t)=>{M.textContent=t},parentNode:M=>M.parentNode,nextSibling:M=>M.nextSibling,querySelector:M=>j.querySelector(M),setScopeId(M,t){M.setAttribute(t,"")},insertStaticContent(M,t,T,N,I,D){const L=T?T.previousSibling:t.lastChild;if(I&&(I===D||I.nextSibling))for(;t.insertBefore(I.cloneNode(!0),T),!(I===D||!(I=I.nextSibling)););else{O.innerHTML=b(N==="svg"?`<svg>${M}</svg>`:N==="mathml"?`<math>${M}</math>`:M);const g=O.content;if(N==="svg"||N==="mathml"){const c=g.firstChild;for(;c.firstChild;)g.appendChild(c.firstChild);g.removeChild(c)}t.insertBefore(g,T)}return[L?L.nextSibling:t.firstChild,T?T.previousSibling:t.lastChild]}},$=Symbol("_vtc");function q(M,t,T){const N=M[$];N&&(t=(t?[t,...N]:[...N]).join(" ")),t==null?M.removeAttribute("class"):T?M.setAttribute("class",t):M.className=t}const E=Symbol("_vod"),v=Symbol("_vsh"),dM={name:"show",beforeMount(M,{value:t},{transition:T}){M[E]=M.style.display==="none"?"":M.style.display,T&&t?T.beforeEnter(M):y(M,t)},mounted(M,{value:t},{transition:T}){T&&t&&T.enter(M)},updated(M,{value:t,oldValue:T},{transition:N}){!t!=!T&&(N?t?(N.beforeEnter(M),y(M,!0),N.enter(M)):N.leave(M,()=>{y(M,!1)}):y(M,t))},beforeUnmount(M,{value:t}){y(M,t)}};function y(M,t){M.style.display=t?M[E]:"none",M[v]=!t}const MM=Symbol(""),tM=/(?:^|;)\s*display\s*:/;function TM(M,t,T){const N=M.style,I=i(T);let D=!1;if(T&&!I){if(t)if(i(t))for(const L of t.split(";")){const g=L.slice(0,L.indexOf(":")).trim();T[g]==null&&x(N,g,"")}else for(const L in t)T[L]==null&&x(N,L,"");for(const L in T)L==="display"&&(D=!0),x(N,L,T[L])}else if(I){if(t!==T){const L=N[MM];L&&(T+=";"+L),N.cssText=T,D=tM.test(T)}}else t&&M.removeAttribute("style");E in M&&(M[E]=D?N.display:"",M[v]&&(N.display="none"))}const d=/\s*!important$/;function x(M,t,T){if(a(T))T.forEach(N=>x(M,t,N));else if(T==null&&(T=""),t.startsWith("--"))M.setProperty(t,T);else{const N=NM(M,t);d.test(T)?M.setProperty(u(N),T.replace(d,""),"important"):M[N]=T}}const r=["Webkit","Moz","ms"],z={};function NM(M,t){const T=z[t];if(T)return T;let N=A(t);if(N!=="filter"&&N in M)return z[t]=N;N=X(N);for(let I=0;I<r.length;I++){const D=r[I]+N;if(D in M)return z[t]=D}return t}const w="http://www.w3.org/1999/xlink";function l(M,t,T,N,I,D=R(t)){N&&t.startsWith("xlink:")?T==null?M.removeAttributeNS(w,t.slice(6,t.length)):M.setAttributeNS(w,t,T):T==null||D&&!H(T)?M.removeAttribute(t):M.setAttribute(t,D?"":W(T)?String(T):T)}function s(M,t,T,N,I){if(t==="innerHTML"||t==="textContent"){T!=null&&(M[t]=t==="innerHTML"?b(T):T);return}const D=M.tagName;if(t==="value"&&D!=="PROGRESS"&&!D.includes("-")){const g=D==="OPTION"?M.getAttribute("value")||"":M.value,c=T==null?M.type==="checkbox"?"on":"":String(T);(g!==c||!("_value"in M))&&(M.value=c),T==null&&M.removeAttribute(t),M._value=T;return}let L=!1;if(T===""||T==null){const g=typeof M[t];g==="boolean"?T=H(T):T==null&&g==="string"?(T="",L=!0):g==="number"&&(T=0,L=!0)}try{M[t]=T}catch{}L&&M.removeAttribute(I||t)}function f(M,t,T,N){M.addEventListener(t,T,N)}function IM(M,t,T,N){M.removeEventListener(t,T,N)}const U=Symbol("_vei");function LM(M,t,T,N,I=null){const D=M[U]||(M[U]={}),L=D[t];if(N&&L)L.value=N;else{const[g,c]=DM(t);if(N){const B=D[t]=jM(N,I);f(M,g,B,c)}else L&&(IM(M,g,L,c),D[t]=void 0)}}const Q=/(?:Once|Passive|Capture)$/;function DM(M){let t;if(Q.test(M)){t={};let N;for(;N=M.match(Q);)M=M.slice(0,M.length-N[0].length),t[N[0].toLowerCase()]=!0}return[M[2]===":"?M.slice(3):u(M.slice(2)),t]}let n=0;const gM=Promise.resolve(),cM=()=>n||(gM.then(()=>n=0),n=Date.now());function jM(M,t){const T=N=>{if(!N._vts)N._vts=Date.now();else if(N._vts<=T.attached)return;G(yM(N,T.value),t,5,[N])};return T.value=M,T.attached=cM(),T}function yM(M,t){if(a(t)){const T=M.stopImmediatePropagation;return M.stopImmediatePropagation=()=>{T.call(M),M._stopped=!0},t.map(N=>I=>!I._stopped&&N&&N(I))}else return t}const m=M=>M.charCodeAt(0)===111&&M.charCodeAt(1)===110&&M.charCodeAt(2)>96&&M.charCodeAt(2)<123,AM=(M,t,T,N,I,D)=>{const L=I==="svg";t==="class"?q(M,N,L):t==="style"?TM(M,T,N):p(t)?Z(t)||LM(M,t,T,N,D):(t[0]==="."?(t=t.slice(1),!0):t[0]==="^"?(t=t.slice(1),!1):iM(M,t,N,L))?(s(M,t,N),!M.tagName.includes("-")&&(t==="value"||t==="checked"||t==="selected")&&l(M,t,N,L,D,t!=="value")):M._isVueCE&&(xM(M,t)||M._def.__asyncLoader&&(/[A-Z]/.test(t)||!i(N)))?s(M,A(t),N,D,t):(t==="true-value"?M._trueValue=N:t==="false-value"&&(M._falseValue=N),l(M,t,N,L))};function iM(M,t,T,N){if(N)return!!(t==="innerHTML"||t==="textContent"||t in M&&m(t)&&V(T));if(t==="spellcheck"||t==="draggable"||t==="translate"||t==="autocorrect"||t==="sandbox"&&M.tagName==="IFRAME"||t==="form"||t==="list"&&M.tagName==="INPUT"||t==="type"&&M.tagName==="TEXTAREA")return!1;if(t==="width"||t==="height"){const I=M.tagName;if(I==="IMG"||I==="VIDEO"||I==="CANVAS"||I==="SOURCE")return!1}return m(t)&&i(T)?!1:t in M}function xM(M,t){const T=M._def.props;if(!T)return!1;const N=A(t);return Array.isArray(T)?T.some(I=>A(I)===N):Object.keys(T).some(I=>A(I)===N)}const k=M=>{const t=M.props["onUpdate:modelValue"]||!1;return a(t)?T=>F(t,T):t},C=Symbol("_assign"),rM={created(M,{value:t},T){M.checked=e(t,T.props.value),M[C]=k(T),f(M,"change",()=>{M[C](EM(M))})},beforeUpdate(M,{value:t,oldValue:T},N){M[C]=k(N),t!==T&&(M.checked=e(t,N.props.value))}};function EM(M){return"_value"in M?M._value:M.value}const zM=["ctrl","shift","alt","meta"],nM={stop:M=>M.stopPropagation(),prevent:M=>M.preventDefault(),self:M=>M.target!==M.currentTarget,ctrl:M=>!M.ctrlKey,shift:M=>!M.shiftKey,alt:M=>!M.altKey,meta:M=>!M.metaKey,left:M=>"button"in M&&M.button!==0,middle:M=>"button"in M&&M.button!==1,right:M=>"button"in M&&M.button!==2,exact:(M,t)=>zM.some(T=>M[`${T}Key`]&&!t.includes(T))},wM=(M,t)=>{if(!M)return M;const T=M._withMods||(M._withMods={}),N=t.join(".");return T[N]||(T[N]=((I,...D)=>{for(let L=0;L<t.length;L++){const g=nM[t[L]];if(g&&g(I,t))return}return M(I,...D)}))},CM={esc:"escape",space:" ",up:"arrow-up",left:"arrow-left",right:"arrow-right",down:"arrow-down",delete:"backspace"},lM=(M,t)=>{const T=M._withKeys||(M._withKeys={}),N=t.join(".");return T[N]||(T[N]=(I=>{if(!("key"in I))return;const D=u(I.key);if(t.some(L=>L===D||CM[L]===D))return M(I)}))},oM=P({patchProp:AM},K);let Y;function aM(){return Y||(Y=h(oM))}const sM=((...M)=>{const t=aM().createApp(...M),{mount:T}=t;return t.mount=N=>{const I=eM(N);if(!I)return;const D=t._component;!V(D)&&!D.render&&!D.template&&(D.template=I.innerHTML),I.nodeType===1&&(I.textContent="");const L=T(I,!1,uM(I));return I instanceof Element&&(I.removeAttribute("v-cloak"),I.setAttribute("data-v-app","")),L},t});function uM(M){if(M instanceof SVGElement)return"svg";if(typeof MathMLElement=="function"&&M instanceof MathMLElement)return"mathml"}function eM(M){return i(M)?document.querySelector(M):M}const SM=new CSSStyleSheet;SM.replaceSync(`@layer kern-ux-icons {
	
		.kern-icon--arrow-back {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--arrow-back {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--arrow-forward {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--ar-stickers {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--open-in-new {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--check {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--content-copy {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--ar-stickers {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--open-in-new {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--home {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--school {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--book-5 {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--open-in-new {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--flag {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--splitscreen-left {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--assignment {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--check {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--layers {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--check {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--location-on {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--palette {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--rocket-launch {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--settings {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--build {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--orbit {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--extension {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--looks {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--museum {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--alt-route {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--map {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--accessibility {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	
}`);const UM=(M,t)=>{const T=M.__vccOpts||M;for(const[N,I]of t)T[N]=I;return T};export{UM as _,dM as a,lM as b,sM as c,SM as s,rM as v,wM as w};

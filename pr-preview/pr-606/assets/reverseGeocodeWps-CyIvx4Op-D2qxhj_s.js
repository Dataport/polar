...
</wps:Execute>`;function t(e,t){return e.getElementsByTagNameNS(`*`,t)[0]?.textContent??``}async function n(n,r,i,a){let o=await fetch(n,{method:`POST`,body:e(r),signal:a}),s=new DOMParser().parseFromString(await o.text(),`text/xml`),c=s.querySelector(`parsererror`);if(c)throw Error(`Failed to parse XML response: ${c.textContent}.`);let l=s.getElementsByTagNameNS(`*`,`Adresse`)[0];if(!l)throw Error(`Response does not contain an "Adresse" element.`);let u={Distanz:parseFloat(t(l,`Distanz`)),Hausnr:parseInt(t(l,`Hausnr`),10),Plz:parseInt(t(l,`Plz`),10),Strasse:t(l,`Strasse`),XKoordinate:parseFloat(t(l,`XKoordinate`)),YKoordinate:parseFloat(t(l,`YKoordinate`)),Zusatz:t(l,`Zusatz`)};return{type:`reverse_geocoded`,title:`${u.Strasse} ${u.Hausnr}${u.Zusatz}`,properties:u,geometry:{coordinates:r,type:`Point`},addressGeometry:{coordinates:[u.XKoordinate,u.YKoordinate],type:`Point`}}}if(import.meta.vitest){let{beforeEach:t,expect:r,test:i,vi:a}=import.meta.vitest;t(()=>{a.restoreAllMocks()});let o=`https://wps.example`,s=[565192.2974622496,5933428.820743558],c=`<?xml version='1.0' encoding='UTF-8'?>
...
</wps:ExecuteResponse>`;i(`reverseGeocode throws on invalid XML`,async()=>{a.spyOn(global,`fetch`).mockResolvedValueOnce({text:()=>Promise.resolve(`<invalid><xml`)}),await r(n(o,s,`EPSG:25832`,new AbortController().signal)).rejects.toThrow(`Failed to parse XML response`)}),i(`reverseGeocode throws when Adresse element is missing`,async()=>{a.spyOn(global,`fetch`).mockResolvedValueOnce({text:()=>Promise.resolve(`<?xml version="1.0"?><root><empty/></root>`)}),await r(n(o,s,`EPSG:25832`,new AbortController().signal)).rejects.toThrow(`Response does not contain an "Adresse" element.`)}),i(`reverseGeocode works with Hamburg-WPS-style`,async()=>{let t=a.spyOn(global,`fetch`).mockResolvedValueOnce({text:()=>Promise.resolve(c)}),i=new AbortController,l=await n(o,s,`EPSG:25832`,i.signal);r(t).toHaveBeenCalledOnce(),r(t).toHaveBeenCalledWith(o,{method:`POST`,body:e(s),signal:i.signal}),r(l).toEqual({type:`reverse_geocoded`,title:`Herrlichkeit 1`,addressGeometry:{coordinates:[565200.347,5933442.881],type:`Point`},geometry:{coordinates:s,type:`Point`},properties:{Distanz:16.20141565450446,Hausnr:1,Plz:20459,Strasse:`Herrlichkeit`,XKoordinate:565200.347,YKoordinate:5933442.881,Zusatz:``}})})}export{n as t};
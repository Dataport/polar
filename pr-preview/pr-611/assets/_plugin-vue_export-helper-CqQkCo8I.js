import{i as k,c as B,a as A,e as h,b as f,d as P,f as c,g as Y,h as p,j as W,k as u,l as V,m as Z,n as R,o as n,p as X}from"./runtime-core.esm-bundler-Bxc-1ffD.js";let C;const a=typeof window<"u"&&window.trustedTypes;if(a)try{C=a.createPolicy("vue",{createHTML:M=>M})}catch{}const H=C?M=>C.createHTML(M):M=>M,G="http://www.w3.org/2000/svg",F="http://www.w3.org/1998/Math/MathML",y=typeof document<"u"?document:null,S=y&&y.createElement("template"),J={insert:(M,t,T)=>{t.insertBefore(M,T||null)},remove:M=>{const t=M.parentNode;t&&t.removeChild(M)},createElement:(M,t,T,N)=>{const I=t==="svg"?y.createElementNS(G,M):t==="mathml"?y.createElementNS(F,M):T?y.createElement(M,{is:T}):y.createElement(M);return M==="select"&&N&&N.multiple!=null&&I.setAttribute("multiple",N.multiple),I},createText:M=>y.createTextNode(M),createComment:M=>y.createComment(M),setText:(M,t)=>{M.nodeValue=t},setElementText:(M,t)=>{M.textContent=t},parentNode:M=>M.parentNode,nextSibling:M=>M.nextSibling,querySelector:M=>y.querySelector(M),setScopeId(M,t){M.setAttribute(t,"")},insertStaticContent(M,t,T,N,I,D){const L=T?T.previousSibling:t.lastChild;if(I&&(I===D||I.nextSibling))for(;t.insertBefore(I.cloneNode(!0),T),!(I===D||!(I=I.nextSibling)););else{S.innerHTML=H(N==="svg"?`<svg>${M}</svg>`:N==="mathml"?`<math>${M}</math>`:M);const g=S.content;if(N==="svg"||N==="mathml"){const j=g.firstChild;for(;j.firstChild;)g.appendChild(j.firstChild);g.removeChild(j)}t.insertBefore(g,T)}return[L?L.nextSibling:t.firstChild,T?T.previousSibling:t.lastChild]}},_=Symbol("_vtc");function K(M,t,T){const N=M[_];N&&(t=(t?[t,...N]:[...N]).join(" ")),t==null?M.removeAttribute("class"):T?M.setAttribute("class",t):M.className=t}const O=Symbol("_vod"),$=Symbol("_vsh"),q=Symbol(""),MM=/(?:^|;)\s*display\s*:/;function tM(M,t,T){const N=M.style,I=A(T);let D=!1;if(T&&!I){if(t)if(A(t))for(const L of t.split(";")){const g=L.slice(0,L.indexOf(":")).trim();T[g]==null&&x(N,g,"")}else for(const L in t)T[L]==null&&x(N,L,"");for(const L in T)L==="display"&&(D=!0),x(N,L,T[L])}else if(I){if(t!==T){const L=N[q];L&&(T+=";"+L),N.cssText=T,D=MM.test(T)}}else t&&M.removeAttribute("style");O in M&&(M[O]=D?N.display:"",M[$]&&(N.display="none"))}const o=/\s*!important$/;function x(M,t,T){if(u(T))T.forEach(N=>x(M,t,N));else if(T==null&&(T=""),t.startsWith("--"))M.setProperty(t,T);else{const N=TM(M,t);o.test(T)?M.setProperty(V(N),T.replace(o,""),"important"):M[N]=T}}const d=["Webkit","Moz","ms"],i={};function TM(M,t){const T=i[t];if(T)return T;let N=c(t);if(N!=="filter"&&N in M)return i[t]=N;N=Z(N);for(let I=0;I<d.length;I++){const D=d[I]+N;if(D in M)return i[t]=D}return t}const e="http://www.w3.org/1999/xlink";function w(M,t,T,N,I,D=W(t)){N&&t.startsWith("xlink:")?T==null?M.removeAttributeNS(e,t.slice(6,t.length)):M.setAttributeNS(e,t,T):T==null||D&&!Y(T)?M.removeAttribute(t):M.setAttribute(t,D?"":p(T)?String(T):T)}function r(M,t,T,N,I){if(t==="innerHTML"||t==="textContent"){T!=null&&(M[t]=t==="innerHTML"?H(T):T);return}const D=M.tagName;if(t==="value"&&D!=="PROGRESS"&&!D.includes("-")){const g=D==="OPTION"?M.getAttribute("value")||"":M.value,j=T==null?M.type==="checkbox"?"on":"":String(T);(g!==j||!("_value"in M))&&(M.value=j),T==null&&M.removeAttribute(t),M._value=T;return}let L=!1;if(T===""||T==null){const g=typeof M[t];g==="boolean"?T=Y(T):T==null&&g==="string"?(T="",L=!0):g==="number"&&(T=0,L=!0)}try{M[t]=T}catch{}L&&M.removeAttribute(I||t)}function b(M,t,T,N){M.addEventListener(t,T,N)}function NM(M,t,T,N){M.removeEventListener(t,T,N)}const l=Symbol("_vei");function IM(M,t,T,N,I=null){const D=M[l]||(M[l]={}),L=D[t];if(N&&L)L.value=N;else{const[g,j]=LM(t);if(N){const v=D[t]=jM(N,I);b(M,g,v,j)}else L&&(NM(M,g,L,j),D[t]=void 0)}}const U=/(?:Once|Passive|Capture)$/;function LM(M){let t;if(U.test(M)){t={};let N;for(;N=M.match(U);)M=M.slice(0,M.length-N[0].length),t[N[0].toLowerCase()]=!0}return[M[2]===":"?M.slice(3):V(M.slice(2)),t]}let z=0;const DM=Promise.resolve(),gM=()=>z||(DM.then(()=>z=0),z=Date.now());function jM(M,t){const T=N=>{if(!N._vts)N._vts=Date.now();else if(N._vts<=T.attached)return;R(yM(N,T.value),t,5,[N])};return T.value=M,T.attached=gM(),T}function yM(M,t){if(u(t)){const T=M.stopImmediatePropagation;return M.stopImmediatePropagation=()=>{T.call(M),M._stopped=!0},t.map(N=>I=>!I._stopped&&N&&N(I))}else return t}const s=M=>M.charCodeAt(0)===111&&M.charCodeAt(1)===110&&M.charCodeAt(2)>96&&M.charCodeAt(2)<123,cM=(M,t,T,N,I,D)=>{const L=I==="svg";t==="class"?K(M,N,L):t==="style"?tM(M,T,N):f(t)?P(t)||IM(M,t,T,N,D):(t[0]==="."?(t=t.slice(1),!0):t[0]==="^"?(t=t.slice(1),!1):AM(M,t,N,L))?(r(M,t,N),!M.tagName.includes("-")&&(t==="value"||t==="checked"||t==="selected")&&w(M,t,N,L,D,t!=="value")):M._isVueCE&&(xM(M,t)||M._def.__asyncLoader&&(/[A-Z]/.test(t)||!A(N)))?r(M,c(t),N,D,t):(t==="true-value"?M._trueValue=N:t==="false-value"&&(M._falseValue=N),w(M,t,N,L))};function AM(M,t,T,N){if(N)return!!(t==="innerHTML"||t==="textContent"||t in M&&s(t)&&k(T));if(t==="spellcheck"||t==="draggable"||t==="translate"||t==="autocorrect"||t==="sandbox"&&M.tagName==="IFRAME"||t==="form"||t==="list"&&M.tagName==="INPUT"||t==="type"&&M.tagName==="TEXTAREA")return!1;if(t==="width"||t==="height"){const I=M.tagName;if(I==="IMG"||I==="VIDEO"||I==="CANVAS"||I==="SOURCE")return!1}return s(t)&&A(T)?!1:t in M}function xM(M,t){const T=M._def.props;if(!T)return!1;const N=c(t);return Array.isArray(T)?T.some(I=>c(I)===N):Object.keys(T).some(I=>c(I)===N)}const Q=M=>{const t=M.props["onUpdate:modelValue"]||!1;return u(t)?T=>X(t,T):t},E=Symbol("_assign"),oM={created(M,{value:t},T){M.checked=n(t,T.props.value),M[E]=Q(T),b(M,"change",()=>{M[E](iM(M))})},beforeUpdate(M,{value:t,oldValue:T},N){M[E]=Q(N),t!==T&&(M.checked=n(t,N.props.value))}};function iM(M){return"_value"in M?M._value:M.value}const zM=["ctrl","shift","alt","meta"],EM={stop:M=>M.stopPropagation(),prevent:M=>M.preventDefault(),self:M=>M.target!==M.currentTarget,ctrl:M=>!M.ctrlKey,shift:M=>!M.shiftKey,alt:M=>!M.altKey,meta:M=>!M.metaKey,left:M=>"button"in M&&M.button!==0,middle:M=>"button"in M&&M.button!==1,right:M=>"button"in M&&M.button!==2,exact:(M,t)=>zM.some(T=>M[`${T}Key`]&&!t.includes(T))},dM=(M,t)=>{if(!M)return M;const T=M._withMods||(M._withMods={}),N=t.join(".");return T[N]||(T[N]=((I,...D)=>{for(let L=0;L<t.length;L++){const g=EM[t[L]];if(g&&g(I,t))return}return M(I,...D)}))},CM=h({patchProp:cM},J);let m;function uM(){return m||(m=B(CM))}const eM=((...M)=>{const t=uM().createApp(...M),{mount:T}=t;return t.mount=N=>{const I=aM(N);if(!I)return;const D=t._component;!k(D)&&!D.render&&!D.template&&(D.template=I.innerHTML),I.nodeType===1&&(I.textContent="");const L=T(I,!1,nM(I));return I instanceof Element&&(I.removeAttribute("v-cloak"),I.setAttribute("data-v-app","")),L},t});function nM(M){if(M instanceof SVGElement)return"svg";if(typeof MathMLElement=="function"&&M instanceof MathMLElement)return"mathml"}function aM(M){return A(M)?document.querySelector(M):M}const SM=new CSSStyleSheet;SM.replaceSync(`@layer kern-ux-icons {
	
		.kern-icon--arrow-back {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--arrow-back {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--arrow-forward {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--flag {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--splitscreen-left {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--dark-mode {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--assignment {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--check {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--layers {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--ar-stickers {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--open-in-new {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--check {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--content-copy {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--check-circle {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon-fill--ar-stickers {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--open-in-new {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--home {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--school {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--book-5 {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--open-in-new {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--check {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--location-on {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--palette {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--rocket-launch {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--settings {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--build {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--orbit {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--extension {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--looks {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--museum {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--alt-route {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--map {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--accessibility {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--groups {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	

		.kern-icon--bug-report {
			mask: url("data:image/svg+xml;base64,");
			background-color: var(--kern-color-layout-text-default, #171a2b);
		}
	
}`);const wM=(M,t)=>{const T=M.__vccOpts||M;for(const[N,I]of t)T[N]=I;return T};export{wM as _,eM as c,SM as s,oM as v,dM as w};

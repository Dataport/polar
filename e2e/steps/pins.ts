...
import type { CanvasBox } from './context'
...
const loadingIndicator = (page: Page) =>
	page.locator(LOADING_INDICATOR_SELECTOR).first()

/** The reverse geocoding request can resolve before the spinner is observed. */
async function waitForLoadingStart(page: Page): Promise<void> {
	await loadingIndicator(page)
		.waitFor({ state: 'visible', timeout: 2000 })
		.catch(() => undefined)
}

async function waitForLoadingEnd(page: Page): Promise<void> {
	await loadingIndicator(page).waitFor({ state: 'hidden', timeout: 10_000 })
}

async function ensureZoomLevel(page: Page, targetLevel: number) {
	const zoomTarget = page.locator(ZOOM_STATE_SELECTOR)
	...
	while (current !== targetLevel && attempts < 20) {
		const label = current < targetLevel ? ZOOM_IN_LABEL : ZOOM_OUT_LABEL
...
Given('no pin coordinate is set', async ({ page }) => {
	...
	const coordinateTarget = page.locator(PIN_COORDINATE_STATE_SELECTOR)
	await expect(coordinateTarget).toHaveText('null')
 ...
 *
 * @param level - The target zoom level to set on the map.
 */

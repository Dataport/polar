...
const expectationsByTestClientUuid = new Map<string, MockExpectation[]>()

/** Received-request queues keyed by test-client UUID (chronological per UUID). */
const receivedRequestsByTestClientUuid = new Map<string, ReceivedRequest[]>()
...
function pruneStaleEntries(): void {
	const now = Date.now()
	for (const [testClientUuid, expectations] of expectationsByTestClientUuid) {
		const active = expectations.filter((e) => now - e.createdAt < e.ttlMs)
		if (active.length === 0) {
			expectationsByTestClientUuid.delete(testClientUuid)
		} else {
			expectationsByTestClientUuid.set(testClientUuid, active)
		}
	}

	for (const [testClientUuid, requests] of receivedRequestsByTestClientUuid) {
		const recent = requests.filter((r) => now - r.receivedAt < DEFAULT_TTL_MS)
		if (recent.length === 0) {
			receivedRequestsByTestClientUuid.delete(testClientUuid)
		} else {
			receivedRequestsByTestClientUuid.set(testClientUuid, recent)
		}
	}
}
...
function getExpectationsQueue(testClientUuid: string): MockExpectation[] {
	let queue = expectationsByTestClientUuid.get(testClientUuid)
	if (!queue) {
		queue = []
		expectationsByTestClientUuid.set(testClientUuid, queue)
	}
	return queue
}
...
function getReceivedRequestsQueue(testClientUuid: string): ReceivedRequest[] {
	let queue = receivedRequestsByTestClientUuid.get(testClientUuid)
	if (!queue) {
		queue = []
		receivedRequestsByTestClientUuid.set(testClientUuid, queue)
	}
	return queue
}
...
function getAllExpectations(): MockExpectation[] {
	return [...expectationsByTestClientUuid.values()].flat()
}
...
function getAllReceivedRequests(): ReceivedRequest[] {
	return [...receivedRequestsByTestClientUuid.values()].flat()
}
...
function matchesRequest(
	...
	if (matcher.query) {
		for (const [key, expected] of Object.entries(matcher.query)) {
			if (!matchesValue(query[key], expected)) {
				return false
			}
		}
	}
...
function findMatchingExpectation(
	...
	const expectations = expectationsByTestClientUuid.get(testClientUuid) ?? []
...
function sendMockResponse(
	...
	// typed as unknown so the runtime null/undefined guards below are honest
	const rawBody: unknown = mockRes.body
...
function reviveRegExp(obj: unknown): unknown {
	...
	if ('__regexp' in rec && typeof rec.__regexp === 'string') {
		const flags = typeof rec.__flags === 'string' ? rec.__flags : ''
		return new RegExp(rec.__regexp, flags)
	}
...
async function handleAdmin(
	...
	if (pathname === '/__mock/expectations' && req.method === 'GET') {
		sendJson(
			res,
			scopedUuid
				? (expectationsByTestClientUuid.get(scopedUuid) ?? [])
				: getAllExpectations()
		)
		return
	}
	...
	if (pathname === '/__mock/requests' && req.method === 'GET') {
		sendJson(
			res,
			scopedUuid
				? (receivedRequestsByTestClientUuid.get(scopedUuid) ?? [])
	...
	if (pathname === '/__mock/requests/count' && req.method === 'GET') {
		sendJson(res, {
			count: scopedUuid
				? (receivedRequestsByTestClientUuid.get(scopedUuid) ?? []).length
	...
	if (pathname === '/__mock/expectations' && req.method === 'DELETE') {
		if (scopedUuid) {
			expectationsByTestClientUuid.delete(scopedUuid)
		} else {
			expectationsByTestClientUuid.clear()
		}
	...
	if (pathname === '/__mock/requests' && req.method === 'DELETE') {
		if (scopedUuid) {
			receivedRequestsByTestClientUuid.delete(scopedUuid)
		} else {
			receivedRequestsByTestClientUuid.clear()
		}
		sendJson(res, { cleared: true })
		return
	}
...
async function handleRequest(
	...
	if (match) {
		...
		if (!match.persistent) {
			const remaining = (
				expectationsByTestClientUuid.get(testClientUuid) ?? []
			).filter((e) => e.id !== match.id)

			if (remaining.length === 0) {
				expectationsByTestClientUuid.delete(testClientUuid)
			} else {
				expectationsByTestClientUuid.set(testClientUuid, remaining)
			}
		}
